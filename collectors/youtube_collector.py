...
def resolve_channel_id(channel_id_or_handle: str, api_key: str) -> str:
    ...
    try:
        ...
        if items:
            resolved = items[0]["snippet"]["channelId"]
            print(f"    [ID 변환] {channel_id_or_handle} -> {resolved}")
            return resolved
    except Exception as e:
        print(f"    [ID 변환 실패] {channel_id_or_handle}: {e}")
    return channel_id_or_handle
...
def get_transcript(video_id: str) -> str:
    ...
    try:
        ...
        try:
            from youtube_transcript_api import YouTubeTranscriptApi
            transcript_list = YouTubeTranscriptApi.get_transcript(video_id, languages=["ko-auto"])
            return " ".join([t["text"] for t in transcript_list])
        except Exception:
            return ""
...
def collect_broadcast_youtube(broadcast_channels: dict, api_key: str, hours: int = 24) -> list:
    ...
    for channel_name, channel_id in broadcast_channels.items():
        ...
        actual_id = resolve_channel_id(channel_id, api_key)
        videos = get_recent_videos(actual_id, api_key, hours=hours, max_results=15)

        for video in videos:
            title = video["title"]
            keywords = ["종목","주식","코스피","코스닥","매수","매도","상승","급등",
                       "테마","섹터","반도체","2차전지","AI","로봇","바이오","실적",
                       "목표가","추천","전망","분석","투자","증시","시황","마감",
                       "승부주","관심주","수혜주","대장주","ETF","삼성","SK","LG",
                       "현대","카카오","네이버","배터리","자동차","조선","방산","원전","금리"]
...
def collect_youtuber(youtuber_channels: dict, api_key: str, hours: int = 48) -> list:
    ...
    for channel_name, channel_id in youtuber_channels.items():
        print(f"  수집 중: {channel_name} (최근 {hours}시간)")
        actual_id = resolve_channel_id(channel_id, api_key)
        videos = get_recent_videos(actual_id, api_key, hours=hours, max_results=10)
